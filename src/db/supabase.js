...
async function saveWwebjsSession(clientId, base64Zip) {
  var { error } = await supabase
    .from('whatsapp_sessions')
    .upsert({
      client_id:    clientId,
      session_data: base64Zip,
      updated_at:   new Date().toISOString()
    }, { onConflict: 'client_id' });
  if (error) throw error;
}

async function loadWwebjsSession(clientId) {
  var result = await supabase
    .from('whatsapp_sessions')
    .select('session_data')
    .eq('client_id', clientId)
    .not('session_data', 'is', null)
    .maybeSingle();
  return result.data ? result.data.session_data : null;
}

async function deleteWwebjsSession(clientId) {
  var { error } = await supabase
    .from('whatsapp_sessions')
...
module.exports = {
  ...
  wwebjsSessionExists, saveWwebjsSession, loadWwebjsSession, deleteWwebjsSession
};
