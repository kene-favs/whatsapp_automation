...
//
//  v3 vs uploaded file — ONLY additions, nothing removed:
//   - PUT  /api/client/bot-setup        (questionnaire — was missing)
//   - GET  /api/client/settings         (pre-fill settings page)
//   - GET  /api/client/session-status   (WA badge)
...
//   - PUT  /api/client/listings/:id     (full update)
//   - POST /api/client/listings/:id/media upgraded to file upload
//   - PUT  /api/client/settings expanded (welcome_message etc.)
//   - GET  /api/client/listings fetches media in same query
// ============================================================
...
// ══════════════════════════════════════════════════════════════
//  EXISTING ROUTES — preserved exactly from uploaded file
// ══════════════════════════════════════════════════════════════
...
// PUT /api/client/settings  — EXPANDED (welcome_message, fallback_message added)
...
// ══════════════════════════════════════════════════════════════
//  NEW ROUTES v2 — preserved from uploaded file
// ══════════════════════════════════════════════════════════════
...
router.put('/client/bot-setup', async function(req, res) {
  try {
    ...
    // 2. Build bot_setup upsert payload
    var setup = {
      client_id:           clientId,
    ...
    // Remove undefined/null/empty so we don't overwrite existing data
    Object.keys(setup).forEach(function(k) {
      if (k !== 'client_id' && (setup[k] === null || setup[k] === undefined || setup[k] === '')) {
        delete setup[k];
      }
    });
...
// GET /api/client/listings  — includes media in same query
...
router.post('/client/listings', async function(req, res) {
  try {
    ...
    var result = await sb.from('service_listings').insert({
      ...
      listing_type: listing_type || 'product', available: available !== false
    }).select().single();
...
// POST /api/client/listings/:id/media  — accepts file upload OR JSON {url}
router.post('/client/listings/:id/media', upload.single('file'), async function(req, res) {
  try {
    ...
    if (req.file) {
      ...
      var upResult = await sb.storage.from('forgebot-listings').upload(filename, req.file.buffer, { contentType: req.file.mimetype, upsert: false });
      ...
      publicUrl    = url;
      origFilename = req.body.filename || null;
    }

    var result = await sb.from('listing_media').insert({
      listing_id: req.params.id, client_id: req.clientId,
      url: publicUrl, media_type: mediaType,
      caption: req.body.caption || null, filename: origFilename,
  ...
  try {
    ...
    res.json({
      ...
      bot_setup: {
        ...
        payment_methods:    Array.isArray(setup.payment_methods) ? setup.payment_methods.join(',') : (setup.payment_methods || ''),
...
// GET /api/client/session-status  — powers the WA badge in dashboard header
...
router.get('/client/analytics', async function(req, res) {
  try {
    ...
    if (!month) { var n = new Date(); month = n.getFullYear() + '-' + String(n.getMonth() + 1).padStart(2, '0'); }
    ...
    var revenue  = conf.reduce(function(s, o) { return s + (parseFloat(o.total) || 0); }, 0);
    ...
    res.json({ new_customers: custRes.count || 0, leads: leadsRes.count || 0, orders_placed: placed, orders_confirmed: conf.length, total_revenue: revenue });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
router.get('/client/broadcast-logs', async function(req, res) {
  try {
    var sb     = getSupabase();
    var result = await sb.from('broadcasts').select('*').eq('client_id', req.clientId).order('sent_at', { ascending: false }).limit(20);
    res.json(result.data || []);
  } catch (e) { res.status(500).json({ error: e.message }); }
});

// POST /api/client/broadcast  — targeted phones list
router.post('/client/broadcast', async function(req, res) {
  try {
    ...
    for (var i = 0; i < phoneList.length; i++) {
      ...
      try { await sock.sendMessage(jid, { text: message }); sent++; await new Promise(function(r) { setTimeout(r, 1200); }); }
      catch (e) { console.error('[Broadcast] Failed for', jid); }
    }
    var sb = getSupabase();
    await sb.from('broadcasts').insert({ client_id: req.clientId, message: message, recipients: sent, sent_at: new Date().toISOString() });
...
router.post('/client/bot-tasks', async function(req, res) {
  try {
    ...
    if (!name || !message || !schedule_time || !schedule_days) return res.status(400).json({ error: 'name, message, schedule_time and schedule_days are required' });
    var sb     = getSupabase();
    var result = await sb.from('bot_tasks').insert({ client_id: req.clientId, name: name, message: message, schedule_time: schedule_time, schedule_days: schedule_days, filter_type: filter_type || 'all_customers', active: true, run_count: 0 }).select().single();
    if (result.error) throw new Error(result.error.message);
    res.json(result.data);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
router.get('/client/post-schedule', async function(req, res) {
  try {
    var sb     = getSupabase();
    var result = await sb.from('bot_setup').select('post_schedule_days,post_schedule_time,post_include_memes').eq('client_id', req.clientId).single();
...
router.put('/client/post-schedule', async function(req, res) {
  try {
    ...
    var sb = getSupabase();
    await sb.from('bot_setup').upsert({ client_id: req.clientId, post_schedule_days: days || [], post_schedule_time: time || null, post_include_memes: include_memes !== false, updated_at: new Date().toISOString() }, { onConflict: 'client_id' });
    res.json({ ok: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
router.post('/client/push/subscribe', async function(req, res) {
  try {
    ...
    var sb = getSupabase();
    await sb.from('push_subscriptions').upsert({ client_id: req.clientId, subscription: JSON.stringify({ endpoint: endpoint, keys: keys }), updated_at: new Date().toISOString() }, { onConflict: 'client_id' });
    res.json({ ok: true });
  } catch (e) { res.json({ ok: true }); } // non-fatal if table missing
});

router.post('/client/push/test', async function(req, res) { res.json({ ok: true }); });
