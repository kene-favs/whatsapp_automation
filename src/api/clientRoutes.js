...
//  Auth: JWT via Authorization: Bearer <token>  OR  ?token= for SSE
//
//  New in v3 (badge + bot fixes):
//   - GET  /api/client/session-status   ← THE CRITICAL FIX (was 404)
//   - GET  /api/client/settings         ← dashboard loadSettings() was 404
//   - PUT  /api/client/settings         ← added welcome_message + fallback_message
//   - GET  /api/client/orders           + PUT /api/client/orders/:id
//   - GET  /api/client/analytics
//   - PUT  /api/client/bot-setup
//   - POST /api/client/broadcast        (alias of /broadcasts)
//   - GET  /api/client/broadcast-logs
//   - CRUD /api/client/bot-tasks
//   - GET  /api/push/vapid-key
//   - POST /api/client/push/subscribe
//   - POST /api/client/push/test
//   - qr-stream: removed res.end() on already-connected, added 25s heartbeat
...
// ══════════════════════════════════════════════════════════════
//  THE CRITICAL FIX — was causing badge to always show "Disconnected"
//  Dashboard checkConnectionStatus() and loadSettings() both call this.
//  When it returned 404 JSON, st.connected was undefined → "Disconnected".
// ══════════════════════════════════════════════════════════════
...
// GET /api/client/settings  — was 404 → fixed here
router.get('/client/settings', async function(req, res) {
  try {
    var sb     = getSupabase();
    var result = await sb.from('clients')
      .select('notification_number,business_name,bank_name,account_number,account_name,business_hours,welcome_message,fallback_message,whatsapp_connected')
      .eq('id', req.clientId)
      .single();
    if (result.error) throw new Error(result.error.message);
    var sock = sessionManager.getSession(req.clientId);
    res.json(Object.assign({}, result.data || {}, { whatsapp_connected: !!sock }));
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});
...
// GET /api/client/broadcasts  (plural — legacy)
...
// GET /api/client/broadcast-logs  (alias — dashboard may call either)
...
// POST /api/client/broadcasts  (plural — legacy)
// POST /api/client/broadcast   (singular — dashboard default)
...
router.get('/client/analytics', async function(req, res) {
  try {
    ...
    var start = month + '-01';
    var end   = new Date(new Date(start).getFullYear(), new Date(start).getMonth() + 1, 1).toISOString().slice(0, 10);

    var [msgRes, broadRes, orderRes] = await Promise.all([
    ...
    var totalBroadcast = (broadRes.data || []).reduce(function(sum, r) { return sum + (r.sent_count || 0); }, 0);

    res.json({
      month:            month,
      messages_handled: msgRes.count || 0,
      broadcasts_sent:  totalBroadcast,
...
router.put('/client/bot-setup', async function(req, res) {
  try {
    ...
    var result = await sb.from('bot_setup').upsert(update, { onConflict: 'client_id' }).select().single();
    if (result.error) throw new Error(result.error.message);
    res.json(result.data);
  } catch (e) {
    res.status(500).json({ error: e.message });
...
router.post('/client/bot-tasks', async function(req, res) {
  try {
    ...
    var result = await sb.from('bot_tasks').insert(
      Object.assign({}, req.body, { client_id: req.clientId })
    ).select().single();
    if (result.error) throw new Error(result.error.message);
    res.json(result.data);
  } catch (e) {
...
// GET /api/push/vapid-key  (no auth — browser needs this before login)
...
router.post('/client/push/test', async function(req, res) {
  try {
    // Basic test: just confirm subscription is saved
    var sb     = getSupabase();
    var result = await sb.from('push_subscriptions')
      .select('subscription')
      .eq('client_id', req.clientId)
      .maybeSingle();
    if (!result.data) return res.status(404).json({ error: 'No subscription found — register first' });
    res.json({ ok: true, message: 'Push subscription exists' });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});
...
// ══════════════════════════════════════════════════════════════
//  SSE — QR stream  (fixed: no res.end() + 25s heartbeat)
// ══════════════════════════════════════════════════════════════

// GET /api/client/qr-stream?token=JWT
// Note: auth middleware already set req.clientId via ?token= fallback above
router.get('/client/qr-stream', async function(req, res) {
  ...
  // Heartbeat keeps SSE alive through Railway's proxy (30s idle timeout)
  ...
  // If already connected, tell the client — but DO NOT call res.end()
  // The SSE must stay open so the dashboard can hear future events.
  ...
  if (existingSock) {
    ...
    return; // SSE stays alive — do NOT close
  }
...
router.get('/client/listings', async function(req, res) {
  try {
    var sb     = getSupabase();
    // Include listing_media so dashboard can show listing images
